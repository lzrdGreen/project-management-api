...
class MilestoneForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.project: 
            ...
            if self.instance and self.instance.pk:
                ...
                self.fields['tasks'].queryset = tasks_qs.filter(milestone__isnull=True)
    ...
    def save(self, commit=True):
        ...
        print(f"DEBUG: selected_tasks type = {type(selected_tasks)}")
        if selected_tasks:
            print(f"DEBUG: selected_tasks count = {selected_tasks.count()}")
            for task in selected_tasks:
                print(f"  DEBUG: Task {task.id} - {task.title}")
                
            if selected_tasks is not None:
                # Get the IDs from the queryset
                selected_task_ids = list(selected_tasks.values_list('id', flat=True))
                print(f"DEBUG: selected_task_ids = {selected_task_ids}")
                
                # Remove this milestone from tasks that were deselected
                Task.objects.filter(milestone=milestone).exclude(id__in=selected_task_ids).update(milestone=None)
                
                # Assign this milestone to selected tasks
                Task.objects.filter(id__in=selected_task_ids).update(milestone=milestone)
